...
export class AddNewProductComponent implements OnInit {
    ...
    this.appService.manageHttp('get', 'countries', '').subscribe(res => {
      if (res && res.countrys && res.countrys.length && res.countrys.length > 0) {
        ...
        for (let i = 0; i < allcountries.length; i++) {
          this.countrys.push({ label: allcountries[i].name, value: allcountries[i].countryCode })
        }
      }
    });
  }
  ...
  prepareProductDataSection(productForm?: any) {
    ...
    if (this.appService.loginVendorDetails) {
      this.productData.ownerId = this.appService.loginVendorDetails._id;
      this.productData.ownerName = this.appService.loginVendorDetails.displayName;
    }
    ...
    //     // if (image.signleImage) {
    //     //    image.signleImage = image.signleImage.replace(this.appConfig.imageUrl + 'entity/s/', ' ').trim();
    //     //  }
    //   });
    // });
    ...
    this.appService.manageHttp('get', 'entities/' + productId, '').subscribe((res) => {
      if (res && res.details) {
        ...
        if (this.product.ownerName) {
          this.merchantDetails = { displayName: this.product.ownerName, _id: this.product.ownerId };
        }
  ...
  //Requesting New Category For admin
  requestNewCategory() {
    if (!this.requstCategory) {
      this.submitted = true;
      return
    } else {
      this.appService.loaderStatus('block');
      this.requstCategory = this.appService.capitalize(this.requstCategory)
      let data = { "category": this.requstCategory }
      this.appService.manageHttp('post', 'categoryRequest', data).subscribe(res => {
        if (res && res.respMessage) {
          this.submitted = false;
          $('#reqCatModal').modal('hide');
          this.appService.loaderStatus('none');
          this.appService.displayToasterMessage(res.respMessage);
  ...
  //For Requesting new category
  onClickAddNewRequestCategory() {
    this.requstCategory = '';
    this.submitted = false;
    $('#reqCatModal').modal('show');

  }
  //For Countries List For Autocomplete
  ...
  //For Using Default address
  onClickUseAddress() {
    if (this.appService.loginVendorDetails && this.appService.loginVendorDetails.address) {
      this.address = this.appService.loginVendorDetails.address;
      if (this.address.country && !this.address.country.name) {
        this.address.country = { "name": this.address.country };
      }
    } else {
      this.appService.displayToasterMessage("Default address not available", 'error');
    }
  }
