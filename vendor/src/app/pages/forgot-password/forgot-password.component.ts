...
export class ForgotPasswordComponent {
  ...
  onSubmitForgotPassword(form?: any) {
    if (form && form.status && form.statuS === 'INVALID') {
      ...
      this.appService.manageHttp('post', 'sellers/forgotPassword?displayName=' + form.value.email, '')
        .subscribe((res) => {
  ...
  resetPassword(){
    if(this.resetForm && this.resetForm.status && this.resetForm.status == 'VALID'){
      ...
      this.appService.manageHttp('post','sellers/changeRecoverPassword',this.resetForm.value).subscribe(res =>{
