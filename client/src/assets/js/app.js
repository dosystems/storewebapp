/*-----------------------------------------------------------------------------------
	...
	Theme Name: Ross HTML5 eCommerce Template
	Author: BigSteps
...
$(function () {
		...
		function createImageGallery() {
			...
			$('.product-previews-carousel img').each(function () {
				...
				if(imagepath && imagepath.length>0){
					...
					if(imagepath && imagepath[imagepath.length-2]){
						imagepath.splice(imagepath.length-2, 1,'l');
	...
	function addToBookmark(link) {
		var $link = $(link);
		var isBookmarked = document.cookie.replace(/(?:(?:^|.*;\s*)Rossbookmark\s*\=\s*([^;]*).*$)|^.*$/, "$1");
		...
		$link.on('click', function (e) {
			...
			document.cookie = 'Rossbookmark=yes';
			return false;
		});
	}
	...
	$.fn.stickyHeaderSmart = function () {
		...
		function setHeigth() {
			...
			$('.header-link.header-search').hide();
			$('.megamenu').removeClass('unvisible');
			...
			if ($header.hasClass('sticky')) {
				...
				if (st > lastScrollTop) {
					...
					if (st <= headerOffset) {
						...
						$('.header-link.header-search').show();
						$body.addClass('hdr-sticky');
						$('.megamenu').addClass('unvisible');
					}
				}
				lastScrollTop = st;
			}
	...
	$.fn.initProductZoom = function () {
		...
		if (!$body.is('.touch')) {
			$this.ezPlus({
				...
				zoomWindowWidth: $('.zoomcontainerwidth').outerWidth(),
    			zoomWindowHeight: $('.zoomcontainerwidth').height()-100
			});
		} else {
			$this.ezPlus({
				zoomType: 'lens',
	...
	function loadinitProductZoom(){
		$('.main-image > .zoom').initProductZoom();
	}

	$(document).on('click', '.productdetailsthumbimage, .dblclick-text', function(e){
		if($('.main-image > .zoom').data('ezPlus')){
			$('.main-image > .zoom').data('ezPlus').destroy();
		}
		loadinitProductZoom();
	});
