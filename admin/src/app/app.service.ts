...
export class AppService {
    ...
    setHeaders() {
        let headers = new HttpHeaders()
            ...
            .append('Authorization', "Bearer" + ' ' + localStorage.getItem('employeeToken'));
        return headers;
    }
    ...
    getStatusColor(status: any) {
        ...
        if (status === 'Pending' || status === 'Returned' || status === 'Sales Manager') {
            statusColor = 'warning';
        } else if (status === 'admin' || status === 'Admin' || status === 'Active' || status === 'Delivered' || status === 'Verified') {
            statusColor = 'success';
        } else if (status === 'Inactive' || status === 'Completed' || status === 'Cancelled' || status === 'Rejected' || status === 'Blocked') {
            statusColor = 'danger';
        } else if (status === 'Request' || status === 'Support' || status === 'Shipped' || status === 'AddToCart') {
            statusColor = 'primary';
        } else if (status === 'Processing' || status === 'Paid' || status === 'Sales Representative' || status === 'NotVerified') {
            statusColor = 'info';
        }
        else {
            ...
            if (filterLabels && filterLabels.length > 0) {
                filterLabels.forEach(function (v) {
                    if (filterObj && filterObj[v] && filterObj[v].value) {
                        ...
                        if (v == 'created' || v == 'visibleDate' || v == 'expiryDate' || v == 'startDate' || v == 'endDate' || v == 'deliveryDate') {
                        ...
                        if (v == 'currencies.USD' || v == 'currencies.BUX' || v == 'BUX' || v == 'price' || v == 'actualAmount' || v == 'amount'
                            || v == 'buyRate' || v == 'sellRate') {
                            if (filterObj[v].value.length < 1) {
                                return otherFilterCase = true;
                            }
                        ...
                        if (v == "firstName") {
                            ...
                            if (v !== 'created' && v !== 'visibleDate' && v !== 'expiryDate' && v !== 'inventory.Price' &&
                                ...
                                v !== 'buyRate' && v !== 'sellRate' && v !== 'deliveryDate' && v !== 'currencies.USD' &&
                                v !== 'BUX' && v !== 'currencies.BUX') {
    ...
    // bulk upload
    bulkUpload(type, input) {
        let file;
        if (input && input.files && input.files.length && input.files.length > 0) {
            file = input.files[0];
        }
        ...
        let headers = new HttpHeaders()
            .append('Authorization', "Bearer" + ' ' + localStorage.getItem('employeeToken'));
        return this.http.post(this.appConfig.serverUrl + 'employees/csvupload?type=' + type, formData, { headers: headers })
            .map(respone => {
                return respone;
            });
    }
