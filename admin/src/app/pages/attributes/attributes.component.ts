...
//Get Export Csv
  getExport(){
    this.attributesCsv = [
      { header:"Attribute Name" , field:"name"},
      { header:"Created" , field:"created" },
      { header:"Created By" , field:"createdBy.name" }
    ]
  }
  ...
  getAllAttributes(event?: any) {
    let filterCriteria;
    let filterLabels = ['name', 'created', 'createdBy.name'];
  ...
  onClickAction(type, attribute?: any) {
    if (type === 'add') {
      ...
      if (type === 'edit') {
        ...
        $('#deleteAttribute').modal({ backdrop: 'static', keyboard: false }, 'show');
      }
    }
  }
