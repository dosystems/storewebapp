...
describe('## Check order functionality', () => {
	...
	it('## cancel order :: update order status paid to Cancelled ', (done) => { //mochaAsync(async () => {
		request(app).put('/api/orders/'+orderId).send({status:"Cancelled"})
			.set({ Authorization: `Bearer ${buyer.getAccessToken()}` })
			.expect(httpStatus.OK)
			.then((response) => {
	...
	it('## amount refund order :: update order status Cancelled to Refunded', (done) => { //mochaAsync(async () => {
